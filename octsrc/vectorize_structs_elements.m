...
% Expects cells vector of structures. Combines element from every struct into
% vectors (for scalar elements) or into arrays (for vector elements).
...
% MIT License. 
    ...
    for n = 1:rn
        ...
        if isscalar(fld) && (isnumeric(fld) || isreal(fld))
            ...
            par = zeros(N,1);
            par(1) = fld;
        elseif ~ischar(fld) && isvector(fld) && (isnumeric(fld) || isreal(fld))
            % is vector 
